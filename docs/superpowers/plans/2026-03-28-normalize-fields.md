...
**Tech Stack:** regex (patch), better-sqlite3, vitest, @modelcontextprotocol/sdk (tests)
...
Expected: Tests that check for changed content FAIL (stub returns input unchanged). No-op/edge-case tests pass.
...
Add these two lines at the end of `src/serializer/index.ts` (after line 4):
...
**Files:**
- Modify: `src/sync/watcher.ts:10-22`
- Modify: `src/sync/watcher.ts:67-69` (handleAddOrChange)
- Modify: `src/sync/watcher.ts:99-101` (unlink handler)
...
Expected: FAIL — `acquireGlobalWriteLock` is not exported from `src/sync/watcher.js`

- [ ] **Step 3: Implement global write lock in `src/sync/watcher.ts`**

Add after line 21 (after the `isWriteLocked` function):
...
Add global lock check as the first line of `handleAddOrChange` (before the existing `isWriteLocked` check at line 69):
...
Add global lock check as the first line of the `unlink` handler (before the existing `isWriteLocked` check at line 101):
...
Replace the watcher export line (line 2) with:
...
- Modify: `src/mcp/server.ts:1-30` (imports), `src/mcp/server.ts:1978` (tool registration before `return server`)
- Create: `tests/mcp/normalize-fields.test.ts`

- [ ] **Step 1: Create `src/mcp/normalize-fields.ts` with types and stubs**
...
Add import at line 25 (after the existing `import { resolveEmbeds }` line):

```typescript
import { normalizeFields } from './normalize-fields.js';
```

Add tool registration before `return server;` (before line 1979):
...
In the MCP Layer section (after the `summarize-node` bullet), add:
...
In the Sync Layer watcher description, add after the write lock sentence:
...
In the Serializer section (after `frontmatter.ts`), add:
